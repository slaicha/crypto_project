...
\footnotesize To sign a message: split its hash into digits, and from each chain reveal the value at that digit's position. The verifier finishes every chain and checks all 67 ends match the public key. \textbf{(67 = 64 for 256-bit hash + 3 for checksum)}
\end{block}
\end{frame}
...
\begin{itemize}
    \item The attacker can hash a revealed value forward to forge a valid value for any \emph{higher} digit.
    \item Unprotected, they could forge any message whose digits are all $\geq$ ours.
\end{itemize}

\vspace{4pt}
\textbf{The fix: a checksum} $C = \sum_i (w - 1 - d_i)$, signed with extra chains.
\begin{itemize}
    ...
    \item So the attacker cannot raise a digit without breaking the checksum.
\end{itemize}
...
\end{tabular}
\end{center}
\centering\scriptsize Larger $w$ $\Rightarrow$ fewer chains $\Rightarrow$ smaller signature (but longer chains to compute).
\end{frame}
...
% ---- Slide: Hypertrees (The Lazy Link) ----
\begin{frame}{Hypertrees: The Lazy Link}
\textbf{Question:} How is the top tree built if the bottom roots don't exist yet?

\vspace{6pt}
\begin{columns}
\begin{column}{0.55\textwidth}
\begin{enumerate}
    \item \textbf{Top Tree (Immediate):} 
    \begin{itemize}
        \item Generated from a secret seed.
        \item The leaves are just hashes of \textbf{Top OTS Public Keys}. 
        \item Does \alert{not} contain the bottom root.
    \end{itemize}
    \vspace{6pt}
    \item \textbf{Bottom Tree (Lazy):}
    \begin{itemize}
        \item Built only when signing a message.
        \item Hashes up to a single \textbf{Bottom Root}.
    \end{itemize}
    \vspace{6pt}
    \item \textbf{The Link:}
    \begin{itemize}
        \item The Bottom Root is \textbf{signed} by the corresponding Top OTS Secret Key.
    \end{itemize}
\end{enumerate}
\end{column}
...
\node[box, fill=success!20] (topleaf) at (0, 3) {Top Tree Leaf};
\node[box, fill=accent!20] (toppk) at (0, 1.8) {Top OTS PK};
\node[box, fill=danger!20] (topsk) at (0, 0.4) {Top OTS SK};

\node[box, fill=gray!20] (botroot) at (2.8, 0.4) {Bottom Root};

\draw[->, thick] (toppk) -- (topleaf) node[midway, right, font=\tiny] {$H$};
\draw[->, thick, dashed] (topsk) -- (toppk) node[midway, right, font=\tiny] {Derives};
\draw[->, thick, accent, line width=1pt] (topsk) -- (botroot) node[midway, above, font=\scriptsize\bfseries] {Signs};

\node[font=\tiny, text=danger, below=0.1cm of topsk, align=center] {Generated\\immediately};
\node[font=\tiny, text=gray!80!black, below=0.1cm of botroot, align=center] {Computed\\lazily};
\end{tikzpicture}

\vspace{12pt}
\begin{block}{Verification}
\scriptsize The verifier checks the signature against the Top OTS PK, hashes it to find the Top Leaf, then climbs the top tree.
\end{block}
\end{column}
\end{columns}
\end{frame}
