...
\begin{enumerate}\setcounter{enumi}{4}
    ...
    \item Security: Key-Reuse Attack
    \item Conclusion
\end{enumerate}
\end{column}
\end{columns}
...
% ---- Slide: Security Theory ----
\begin{frame}{Security Analysis: Key-Reuse Attack}
\begin{columns}
\begin{column}{0.55\textwidth}
...
% ---- Slide: Security Plots ----
\begin{frame}{Security Analysis: Empirical Validation}
\begin{columns}
\begin{column}{0.48\textwidth}
\centering
...
\begin{enumerate}
    \item \textbf{Implemented} Lamport OTS, WOTS, and Merkle Signature Scheme from scratch using SHA-256.
...
\begin{itemize}
    ...
    \item MSS KeyGen is $O(2^h)$, but sign/verify are constant and signatures grow by only 32\,B/level.
    \item Hash-based signatures are a viable, well-understood path to post-quantum security.
\end{itemize}
\end{block}

\vspace{4pt}
\textbf{Future work:} SPHINCS+ (stateless), hypertrees, FORS (few-time signatures).
\end{frame}
