\documentclass[10pt,a4paper,twocolumn]{article}

% ── Packages ──
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{geometry}
\geometry{a4paper, margin=0.75in, top=0.9in, bottom=0.9in}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{amsmath,amssymb}
\usepackage{booktabs}
\usepackage{float}
\usepackage{xcolor}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{enumitem}
\usepackage{multicol}

\setlength{\columnsep}{0.3in}
\captionsetup{font=small, labelfont=bf}
\setlist{nosep, leftmargin=*}

% ── Title ──
\title{\vspace{-1.5em}\textbf{Hash-Based Digital Signatures:\\From Lamport OTS to Merkle Signature Schemes}\\[0.3em]
\large Applied Cryptography (CS 232) --- Project Report}
\author{Aicha Slaitane\\\texttt{aicha.slaitane@kaust.edu.sa} \and Abdullah Algethami\\\texttt{abdullah.algethami@kaust.edu.sa}}
\date{}

\begin{document}
\maketitle

% ══════════════════════════════════════════════════════════════
\begin{abstract}
\noindent
We design, implement, and evaluate a complete hash-based digital signature system in Python. Starting from the foundational Lamport One-Time Signature (OTS) scheme, we progress through Winternitz OTS (WOTS) and culminate in a full Merkle Signature Scheme (MSS) that lifts the one-time restriction.

We conduct systematic benchmarks across a wide parameter space---varying the Winternitz parameter $w \in \{4,\dots,256\}$ and tree heights $h \in \{1,\dots,8\}$---to quantify the signature-size versus computation-time trade-off. We demonstrate the critical key-reuse vulnerability of Lamport signatures through a quantitative attack simulation, recovering the full 512-block secret key after only 20 reuses and validating the theoretical forgery probability $(1-2^{-k})^{256}$ against empirical data across 200 independent trials per configuration. Finally, we extend the construction to a two-layer \emph{hypertree}, the same multi-layer Merkle aggregation idea used internally by SPHINCS+/SLH-DSA, and show that it reduces initial KeyGen time by an order of magnitude at matched signing capacity (8.0$\times$ faster at $N\!=\!256$ and 16.2$\times$ faster at $N\!=\!1024$).
\end{abstract}

% ══════════════════════════════════════════════════════════════
\section{Introduction}

Digital signatures are a cornerstone of modern information security, providing authentication, integrity, and non-repudiation. Classical schemes such as RSA and ECDSA derive their security from the computational hardness of integer factorization and the discrete logarithm problem, respectively. However, Shor's algorithm~\cite{shor1994} demonstrates that a sufficiently powerful quantum computer can solve both problems in polynomial time, rendering these schemes insecure in a post-quantum world.

Hash-based signatures offer a fundamentally different security foundation. Their security rests on a single, well-studied assumption: the \emph{one-wayness} and \emph{(second-)preimage resistance} of the underlying cryptographic hash function. The best known quantum speedup against generic hash functions is Grover's algorithm, which provides only a quadratic advantage---reducing an $n$-bit hash from $2^n$ to $2^{n/2}$ security. For SHA-256, this yields approximately 128 bits of quantum security, aligning with NIST's Category-1 post-quantum requirement. This robustness is precisely why NIST standardized the hash-based scheme SLH-DSA (the standardized form of SPHINCS+~\cite{sphincs}) in FIPS 205~\cite{fips205}, alongside the lattice-based ML-DSA, in 2024.

In this project, we implement the fundamental building blocks that underpin modern hash-based signature standards: Lamport OTS, Winternitz OTS, and the Merkle Signature Scheme. All implementations use SHA-256 and are written in pure Python with no external cryptographic libraries. We accompany each construction with unit tests, systematic benchmarks, and a quantitative demonstration of the key-reuse vulnerability inherent in OTS schemes. The complete source code for this project is available on GitHub at \url{https://github.com/slaicha/crypto_project.git}.

% ══════════════════════════════════════════════════════════════
\section{Methods}

\subsection{Lamport One-Time Signatures}
Proposed by Lamport in 1979~\cite{lamport1979}, the scheme generates a secret key consisting of $n$ pairs of random blocks $\{(\text{SK}[i][0],\ \text{SK}[i][1])\}_{i=0}^{n-1}$, where $n$ is the hash output length in bits (256 for SHA-256) and each block is 32 bytes. The public key is the element-wise hash: $\text{PK}[i][b] = H(\text{SK}[i][b])$. To sign a message $m$, the signer computes $h = H(m)$ and reveals $\text{SK}[i][h_i]$ for each bit $h_i$. Verification rehashes each revealed block and compares against the public key. The resulting signature contains 256 blocks of 32 bytes each, totaling 8192 bytes, and the public key is twice that size (16384 bytes) since it stores both hash images per row.

\subsection{Winternitz OTS (WOTS)}
WOTS, first described by Merkle~\cite{merkle1989} and later formally specified within the XMSS standard~\cite{xmss}, generalizes Lamport by processing the message hash in base-$w$ digits rather than individual bits, dramatically reducing signature size. Each digit is signed by its own independent hash chain. The secret key consists of $\ell = \ell_1 + \ell_2$ random start values (chains). For a 256-bit hash (e.g., SHA-256) and $w=16$, the message requires $\ell_1 = 256 / 4 = 64$ chains. 
To prevent forgery where an attacker hashes a revealed value forward to claim a higher digit, WOTS appends a checksum $C = \sum_i (w - 1 - d_i)$. With $w=16$, the maximum sum of 64 digits is $64 \times 15 = 960$, requiring $\ell_2 = 3$ additional base-16 chains to encode ($16^3 = 4096$). Thus, the full key uses exactly $64 + 3 = 67$ parallel chains.
The public key is obtained by hashing each secret block $w-1$ times to reach the chain end: $\text{PK}[i] = H^{w-1}(\text{SK}[i])$. To sign digit $d_i$, the signer reveals the value at position $d_i$ in the chain. The verifier finishes the chain by applying $w-1-d_i$ additional hashes and checks against the public key end. This reduces signature size significantly (e.g., 2,144\,B vs.\ 8,192\,B for Lamport) but increases computation since each chain requires up to $w-1$ hash evaluations.

\subsection{Merkle Signature Scheme}
OTS schemes are inherently single-use. The Merkle tree construction~\cite{merkle1989} overcomes this by generating $N = 2^h$ independent OTS keypairs, hashing each OTS public key into a leaf node, and building a binary hash tree whose root serves as the long-term public key. A signature for the $i$-th message includes: (a) the OTS signature, (b) the OTS public key, and (c) an \emph{authentication path}---the $h$ sibling hashes along the path from leaf $i$ to the root. The verifier checks the OTS signature and reconstructs the root from the leaf using the authentication path, accepting if and only if the reconstructed root matches the published public key. A \texttt{next\_leaf} counter enforces strict one-time usage of each leaf, raising an error upon exhaustion.

\subsection{Multi-Layer Merkle Trees (Hypertrees)}
\label{sec:hypertree}
Section~\ref{sec:experiments} (and Figure~\ref{fig:mss_keygen} in particular) exposed the dominant practical limitation of a flat MSS: KeyGen scales as $O(2^h)$, which becomes prohibitive once we want signing capacities of $2^{16}$ or beyond. We address this by implementing a two-layer \emph{hypertree}, the same multi-layer Merkle construction used internally by SPHINCS+/SLH-DSA.

A two-layer hypertree consists of one \emph{top tree} of height $h_{\text{top}}$ and multiple \emph{bottom trees} of height $h_{\text{bot}}$. The top tree is built \emph{immediately} during initial key generation. Its OTS keypairs are generated deterministically from a secret seed, and its leaves are simply the hashes of these Top OTS public keys. Crucially, the top tree leaves do not contain or depend on the roots of the bottom trees, allowing the top tree to be fully constructed in advance. The root of this top tree serves as the global public key.

The decisive property is that bottom trees are built \emph{lazily}. The cryptographic link between the top tree and a bottom tree is forged dynamically when a signature is requested. The process works as follows:
\begin{enumerate}
    \item \textbf{Materialization}: Based on the message index, the specific bottom tree needed for that index is generated, and its root hash is computed.
    \item \textbf{The Link Signature}: This bottom-tree root is treated as a ``message'' and is signed using the Top OTS secret key corresponding to the leaf directly above that bottom tree. Because the Top OTS public keys are already committed to in the global top root, this signature securely binds the lazily generated bottom tree into the trusted global structure.
    \item \textbf{The Final Bundle}: The resulting hypertree signature bundles four components: (1) the bottom-tree OTS signature on the actual message, (2) the bottom-tree authentication path to reconstruct the bottom root, (3) the Top OTS link signature on the reconstructed bottom root, and (4) the top-tree authentication path to climb from the top leaf to the global top root.
\end{enumerate}
The total signing capacity is $N = 2^{h_{\text{top}} + h_{\text{bot}}}$. This lazy linking turns the up-front cost from $O(2^h)$ into $O(2^{h_{\text{top}}})$, since only the top tree (plus the first bottom tree) is materialized initially. When the two layers are balanced ($h_{\text{top}} \approx h_{\text{bot}} \approx h/2$), this is $O(2^{h/2}) = O(\sqrt{N})$---a square-root reduction in initial key-generation cost. Verification simply walks this chain: it verifies the bottom-tree signature to get the bottom root, then verifies the top-tree signature on that bottom root to climb to the global top root.


% ══════════════════════════════════════════════════════════════
\section{Experimental Results}\label{sec:experiments}

All benchmarks were collected on a standard laptop using SHA-256 as the underlying hash function. Each configuration was measured over 5 independent runs; we report the \emph{mean} execution time together with the \emph{variance} across those runs, so that both the typical cost and the measurement stability are visible. Throughout, the mean is given in milliseconds and the variance (in ms\textsuperscript{2}) is shown after a $\pm$ sign as a compact stability indicator. The consistently small variances confirm that the measurements are stable and repeatable. A single fixed string (\texttt{"Post-quantum cryptography is exciting"}) was signed repeatedly to ensure a fair, controlled comparison across all configurations.

% ══════════════════════════════════════════════════════════════
\subsection{Lamport Key-Reuse Attack}

\subsubsection{Threat Model and Theoretical Analysis}

A Lamport secret key comprises $256 \times 2 = 512$ blocks. Each signature reveals exactly 256 blocks (one per row, selected by the corresponding message-hash bit). After $k$ reuses with independent random messages, the probability that a \emph{specific} block remains unrevealed is $(1/2)^k$. To forge a signature on an arbitrary target, the attacker needs all 256 blocks selected by the target's hash. The forgery probability is:
\begin{equation}
    P_{\text{forge}}(k) = \bigl(1 - 2^{-k}\bigr)^{256}
    \label{eq:forge}
\end{equation}

We verified Eq.~\eqref{eq:forge} empirically by running 200 independent trials per value of $k$, each with a fresh keypair (Table~\ref{tab:forgery}). As shown in Figure~\ref{fig:forgery}, the empirical forgery rate tracks the theoretical curve closely. After only $\sim$10 reuses, the attacker succeeds with $>$78\% probability; after 15, success is essentially certain. The attack is entirely \emph{passive}---the attacker only needs to observe legitimate signatures without any interaction with the signer.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{rccr}
\toprule
$k$ & \textbf{Theoretical} & \textbf{Empirical} & \textbf{Exposed} \\
\midrule
1  & 0.000 & 0.000 & 50.0\% \\
4  & 0.000 & 0.000 & 93.9\% \\
6  & 0.018 & 0.030 & 98.5\% \\
8  & 0.367 & 0.385 & 99.6\% \\
10 & 0.779 & 0.835 & 99.9\% \\
12 & 0.939 & 0.950 & 100.0\% \\
14 & 0.984 & 1.000 & 100.0\% \\
20 & 1.000 & 1.000 & 100.0\% \\
\bottomrule
\end{tabular}
\caption{Forgery probability vs.\ key reuses $k$. ``Exposed'' is the average fraction of all 512 secret blocks revealed to the attacker.}
\label{tab:forgery}
\end{table}

Figure~\ref{fig:bits} provides a complementary view: it plots the average fraction of the 512-block secret key exposed as a function of $k$. By $k\!=\!8$, over 99.6\% of blocks are revealed, and by $k\!=\!12$ the entire key is typically exposed. This highlights that even a small number of reuses leaks nearly all secret material.

\begin{figure*}[tbp]
\centering
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/forgery_probability.png}
    \caption{Forgery success rate vs.\ $k$.}
    \label{fig:forgery}
\end{subfigure}
\hfill
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/key_exposure.png}
    \caption{Fraction of secret-key blocks exposed vs.\ reuse count $k$.}
    \label{fig:bits}
\end{subfigure}
\caption{Empirical and theoretical metrics for the Lamport key-reuse attack over $k \in \{1,\dots,20\}$ reuses (200 independent trials per configuration).}
\label{fig:lamport_attack}
\end{figure*}

\subsubsection{Full Secret Key Recovery}
We explicitly demonstrated secret-key recovery by signing 20 random messages with the same Lamport key. All 512 blocks were recovered byte-exact, as verified against the original key. The full recovered key was exported to JSON for inspection. Once in possession of the complete secret key, the attacker can forge a valid signature on \emph{any} message.

\subsection{OTS Primitive Comparison}

We benchmarked Lamport OTS and WOTS across key Winternitz parameters ($w=4, 16, 64, 256$). Table~\ref{tab:ots_full} summarizes the key generation, signing, verification times, and key/signature sizes.

\begin{table*}[t]
\centering
\small
\begin{tabular}{lrrrrr}
\toprule
\textbf{Scheme} & \textbf{KG (ms)} & \textbf{Sign (ms)} & \textbf{Verify (ms)} & \textbf{Sig (B)} & \textbf{PK (B)} \\
\midrule
Lamport          & $0.74 \pm 0.013$ & $0.02 \pm 0.000$ & $0.13 \pm 0.002$ & 8192  & 16384 \\
WOTS $w\!=\!4$   & $0.29 \pm 0.000$ & $0.11 \pm 0.000$ & $0.10 \pm 0.000$ & 4256  & 4256  \\
WOTS $w\!=\!16$  & $0.40 \pm 0.000$ & $0.19 \pm 0.000$ & $0.18 \pm 0.000$ & 2144  & 2144  \\
WOTS $w\!=\!64$  & $1.06 \pm 0.011$ & $0.43 \pm 0.000$ & $0.55 \pm 0.000$ & 1440  & 1440  \\
WOTS $w\!=\!256$ & $3.00 \pm 0.006$ & $1.56 \pm 0.021$ & $1.56 \pm 0.014$ & 1088  & 1088  \\
\bottomrule
\end{tabular}
\caption{OTS performance across Winternitz parameters. KG = KeyGen, Sig = signature size, PK = public key size. Times are mean $\pm$ variance over 5 runs.}
\label{tab:ots_full}
\end{table*}

The Winternitz trade-off is clearly visible in both Table~\ref{tab:ots_full} and Figures~\ref{fig:ots_sig}--\ref{fig:ots_times}. As $w$ increases, signature size decreases logarithmically (Figure~\ref{fig:ots_sig}): WOTS with $w\!=\!256$ is 7.5$\times$ smaller than Lamport (1088\,B vs.\ 8192\,B). This reduction arises because each doubling of $w$ groups more bits per digit, reducing the number of hash chains $\ell_1$ by a factor proportional to $\log_2 w$. However, computation cost grows linearly with $w$ (Figure~\ref{fig:ots_times}), since each chain is $w-1$ hashes long: WOTS with $w\!=\!256$ is roughly 78$\times$ slower to sign than Lamport. Note that Lamport's signing is extremely fast (0.02\,ms) because it involves no hash computation at all---only selecting and copying precomputed blocks.

\begin{figure*}[tbp]
\centering
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/ots_sig_size.png}
    \caption{Signature size vs.\ $w$.}
    \label{fig:ots_sig}
\end{subfigure}
\hfill
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/ots_times.png}
    \caption{Sign and verify times vs.\ $w$.}
    \label{fig:ots_times}
\end{subfigure}
\caption{OTS performance and scaling trade-offs across Winternitz parameters $w \in \{4, 16, 64, 256\}$ compared to Lamport OTS. Line in (b) shows Lamport verification time (0.13 ms) and signing time (0.02 ms) for reference.}
\label{fig:ots_plots}
\end{figure*}

\subsection{MSS Height Sweep}

Table~\ref{tab:mss_height} reports MSS performance for tree heights $h = 1$ to $8$ using both Lamport and WOTS ($w\!=\!16$) as the leaf OTS.

\begin{table}[htbp]
\centering
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrcr}
\toprule
\textbf{Config} & $N$ & \textbf{KG (ms)} & \textbf{Sign (ms)} & \textbf{Sig (B)} \\
\midrule
$h\!=\!4$, Lamp.  & 16  & $11.96 \pm 0.01$  & $0.03 \pm 0.00$ & 24640 \\
$h\!=\!6$, Lamp.  & 64  & $48.73 \pm 0.71$  & $0.04 \pm 0.00$ & 24704 \\
$h\!=\!8$, Lamp.  & 256 & $196.28 \pm 10.19$ & $0.06 \pm 0.00$ & 24768 \\
\midrule
$h\!=\!4$, WOTS   & 16  & $7.11 \pm 0.03$   & $0.21 \pm 0.00$ & 4420  \\
$h\!=\!6$, WOTS   & 64  & $27.99 \pm 0.17$  & $0.22 \pm 0.00$ & 4484  \\
$h\!=\!8$, WOTS   & 256 & $112.17 \pm 1.13$ & $0.23 \pm 0.00$ & 4548  \\
\bottomrule
\end{tabular}%
}
\caption{MSS performance vs.\ tree height ($N = 2^h$ leaves).}
\label{tab:mss_height}
\end{table}

Three key observations emerge from Table~\ref{tab:mss_height} and Figures~\ref{fig:mss_keygen}--\ref{fig:mss_sigsize}. First, as shown in Figure~\ref{fig:mss_keygen}, KeyGen scales as $O(2^h)$: going from $h\!=\!4$ to $h\!=\!8$ increases the cost by roughly 16$\times$, confirming that the dominant expense is materializing a full OTS keypair at every leaf. Second, Sign times remain effectively constant with respect to $h$ (Figure~\ref{fig:mss_sign}), because the additional $h$ hash operations for the authentication path are negligible compared to the underlying OTS work. Verify times exhibit the exact same constant scaling (Figure~\ref{fig:mss_verify}). Third, signature size grows by only 32 bytes per tree level (Figure~\ref{fig:mss_sigsize})---one SHA-256 sibling hash---meaning that doubling the signing capacity ($h \to h+1$) costs a trivial 32\,B overhead per signature. Across all heights, WOTS leaves produce roughly 5.5$\times$ smaller MSS signatures than Lamport leaves ($\sim$4.5\,KB vs.\ $\sim$24.7\,KB).

\begin{figure*}[tbp]
\centering
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/mss_keygen.png}
    \caption{MSS KeyGen time vs.\ tree height on a log scale.}
    \label{fig:mss_keygen}
\end{subfigure}
\hfill
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/mss_sign.png}
    \caption{MSS Sign time vs.\ height.}
    \label{fig:mss_sign}
\end{subfigure}

\vspace{0.5em}

\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/mss_verify.png}
    \caption{MSS Verify time vs.\ height.}
    \label{fig:mss_verify}
\end{subfigure}
\hfill
\begin{subfigure}{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{benchmarks/mss_sig_size.png}
    \caption{MSS signature size vs.\ height.}
    \label{fig:mss_sigsize}
\end{subfigure}
\caption{MSS performance metrics across tree heights $h \in \{1,\dots,8\}$ for both Lamport and WOTS ($w=16$) leaf backends.}
\label{fig:mss_height_sweep}
\end{figure*}

\subsection{Parameter Tuning: MSS with Varying $w$}

Fixing $h\!=\!4$ (16 leaves), we varied the WOTS parameter to study the size-vs-speed trade-off within the full MSS context (Table~\ref{tab:mss_w}). Going from $w\!=\!4$ to $w\!=\!256$ shrinks signatures by 3.7$\times$ (8644\,B to 2308\,B) but increases total computation (sign + verify) by 13$\times$. Figure~\ref{fig:mss_tradeoff} visualizes this Pareto frontier, showing the smooth and predictable nature of the trade-off curve. The ``knee'' of the curve sits near $w\!=\!16$, which is why real-world standards (XMSS~\cite{xmss}, SPHINCS+~\cite{sphincs}) adopt this value.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{rcccc}
\toprule
$w$ & \textbf{KG (ms)} & \textbf{Sign (ms)} & \textbf{Vrfy (ms)} & \textbf{Sig (B)} \\
\midrule
4   & 4.95  & 0.16 & 0.18 & 8644 \\
16  & 9.09  & 0.30 & 0.30 & 4420 \\
64  & 23.52 & 0.65 & 0.84 & 3012 \\
256 & 70.59 & 2.26 & 2.26 & 2308 \\
\bottomrule
\end{tabular}
\caption{MSS ($h\!=\!4$) performance vs.\ Winternitz parameter $w$.}
\label{tab:mss_w}
\end{table}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\linewidth]{benchmarks/mss_w_tradeoff.png}
\caption{MSS size-vs-speed trade-off at $h\!=\!4$. The knee near $w\!=\!16$ marks the practical sweet spot adopted by XMSS and SPHINCS+.}
\label{fig:mss_tradeoff}
\end{figure}

Based on the complete sweep, Table~\ref{tab:recommendations} summarizes optimal parameter choices for four representative deployment priorities.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Priority} & \textbf{Config} & \textbf{Sig} & \textbf{S+V} & $N$ \\
\midrule
Min size    & $w\!=\!256,h\!=\!4$ & 2308\,B  & 4.52\,ms & 16  \\
Min compute & Lamp., $h\!=\!4$    & 24640\,B & 0.33\,ms & 16  \\
Balanced    & $w\!=\!16, h\!=\!5$ & 4452\,B  & 0.61\,ms & 32  \\
Max msgs    & $w\!=\!16, h\!=\!8$ & 4548\,B  & 0.61\,ms & 256 \\
\bottomrule
\end{tabular}
\caption{Recommended configurations for different priorities (S+V = Sign + Verify time, $N$ = max messages).}
\label{tab:recommendations}
\end{table}

\subsection{Hypertree Performance}
Table~\ref{tab:hypertree} compares the two-layer hypertree against a flat MSS at matched signing capacities. WOTS ($w\!=\!16$) leaves are used throughout. Figure~\ref{fig:hypertree} visualizes the KeyGen comparison.

\begin{table*}[t]
\centering
\small
\begin{tabular}{lrrccr}
\toprule
\textbf{Scheme} & $N$ & \textbf{KG (ms)} & \textbf{Sign (ms)} & \textbf{Vrfy (ms)} & \textbf{Sig (B)} \\
\midrule
Flat MSS, $h\!=\!8$           & 256  & $110.7 \pm 0.6$  & 0.20 & 0.23 & 4548 \\
Hypertree, $h_t\!=\!h_b\!=\!4$ & 256  & $13.8 \pm 0.01$  & 0.17 & 0.60 & 8876 \\
\midrule
Flat MSS, $h\!=\!10$          & 1024 & $444.9 \pm 4.7$  & 0.20 & 0.24 & 4612 \\
Hypertree, $h_t\!=\!h_b\!=\!5$ & 1024 & $27.4 \pm 0.4$   & 0.19 & 0.43 & 8940 \\
\bottomrule
\end{tabular}
\caption{Hypertree vs flat MSS at matched signing capacities. KG = initial KeyGen time.}
\label{tab:hypertree}
\end{table*}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\linewidth]{benchmarks/hypertree_keygen.png}
\caption{Initial KeyGen time: flat MSS vs 2-layer hypertree at matched signing capacities. The hypertree is 8.0$\times$ faster at $N\!=\!256$ and 16.2$\times$ faster at $N\!=\!1024$, consistent with the predicted $O(\sqrt{N})$ reduction.}
\label{fig:hypertree}
\end{figure}

The data is consistent with three predictions. First, KeyGen time drops by roughly the expected $\sqrt{N}$ factor (8.0$\times$ at $N\!=\!256$, 16.2$\times$ at $N\!=\!1024$); the larger speedup at the larger capacity matches the $O(\sqrt{N})$ trend. Second, single-message Sign time is essentially unchanged, because signing still costs one bottom-tree MSS sign plus a precomputed top-tree certificate. Third, signature size roughly doubles ($\sim$4.5\,KB to $\sim$8.9\,KB), since each hypertree signature now carries two MSS signatures instead of one---the standard size-vs-cost trade-off of multi-layer Merkle constructions. This is precisely the engineering trick that lets SPHINCS+ scale to $N\!=\!2^{64}$ while keeping initial KeyGen practical.

% ══════════════════════════════════════════════════════════════
\section{Conclusion}

We implemented a complete hash-based signature pipeline from Lamport OTS through WOTS to the Merkle Signature Scheme, extended it to a two-layer hypertree, and supported all of it with comprehensive benchmarks and a quantitative key-reuse attack. Our key findings are:

\begin{itemize}
    \item WOTS with $w\!=\!16$ offers the best practical balance: as a one-time signature it is 3.8$\times$ smaller than Lamport (2{,}144\,B vs.\ 8{,}192\,B), and when used as the Merkle leaf it yields MSS signatures roughly 5.5$\times$ smaller than Lamport leaves.
    \item Flat MSS KeyGen scales as $O(2^h)$, while signing, verification, and signature size are nearly independent of tree height.
    \item Lamport key reuse is catastrophic: after $\sim$10 reuses, forgery succeeds with $>$78\% probability; after 15 reuses, it is essentially certain. We recovered the full 512-block secret key byte-for-byte after only 20 reuses.
    \item The two-layer hypertree (Section~\ref{sec:hypertree}) reduces initial KeyGen time by 8--16$\times$ at matched signing capacity, at the cost of roughly doubling the signature size---the same engineering trick used by SPHINCS+/SLH-DSA.
\end{itemize}

These results confirm that hash-based signatures are a viable and well-understood path to post-quantum security, requiring only the mild assumption of hash function one-wayness.

% ══════════════════════════════════════════════════════════════
\begin{thebibliography}{9}
\bibitem{shor1994} P.~W.~Shor, ``Algorithms for quantum computation: discrete logarithms and factoring,'' \emph{Proc.\ 35th Annual Symp.\ on Foundations of Computer Science (FOCS)}, pp.~124--134, IEEE, 1994.
\bibitem{lamport1979} L.~Lamport, ``Constructing digital signatures from a one-way function,'' \emph{SRI International, Technical Report CSL-98}, 1979.
\bibitem{merkle1989} R.~C.~Merkle, ``A certified digital signature,'' \emph{Advances in Cryptology --- CRYPTO '89}, LNCS 435, pp.~218--238, Springer, 1990.
\bibitem{xmss} A.~Huelsing, D.~Butin, S.~Gazdag, J.~Rijneveld, and A.~Mohaisen, ``XMSS: eXtended Merkle Signature Scheme,'' \emph{IRTF RFC 8391}, 2018.
\bibitem{sphincs} D.~J.\ Bernstein et al., ``SPHINCS+: Submission to the NIST post-quantum cryptography standardization project,'' original submission, 2017.
\bibitem{fips205} National Institute of Standards and Technology, ``Stateless Hash-Based Digital Signature Standard,'' \emph{FIPS 205 (SLH-DSA)}, 2024.
\end{thebibliography}

\end{document}
